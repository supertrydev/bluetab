...
import { Minus, ChevronDown, ChevronRight, Plus, MoreHorizontal, Archive, Copy, Trash2, LayersPlus } from 'lucide-react';
...
interface UngroupedTabsSectionProps {
    tabs: BrowserTab[];
...
export function UngroupedTabsSection({
    ...
    return (
        <Collapsible open={isOpen} onOpenChange={setIsOpen}>
            <div
                className={cn(
                    "rounded-xl border border-border bg-bg-1 overflow-hidden",
                    isDropTarget && "ring-2 ring-primary/40"
                )}
                onDragOver={(e) => { e.preventDefault(); onContainerDragOver?.(e, 'ungrouped'); }}
                onDrop={(e) => { e.preventDefault(); onContainerDrop?.(e, 'ungrouped'); }}
            >
                {/* Header */}
            ...
            <ConfirmModal
                ...
                title="Delete All Ungrouped Tabs"
                message={`Are you sure you want to close all ${tabs.length} ungrouped tab(s)?`}
                confirmText="Delete"
                cancelText="Cancel"
                onConfirm={() => {
